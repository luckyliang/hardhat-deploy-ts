    ...
    IBaseDoNFT
{
    ...
    uint256 public curDoid; //记录当前DoNFT tokenId
    ...
    /// @notice 根据tokenId和索引获取开始时间和结束时间
    /// @dev index < durationList.length()
    /// @param tokenId a parameter just like in doxygen (must be followed by parameter name)
    /// @param index 小标索引
    /// @return durationId 时间段 id
    /// @return start the return variables of a contract’s function state variable
    /// @return end the return variables of a contract’s function state variable

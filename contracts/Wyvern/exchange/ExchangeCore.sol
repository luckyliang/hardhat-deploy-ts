...
contract ExchangeCore is ReentrancyGuarded, StaticCaller, EIP712 {
    ...
    function validateOrderAuthorization(
        ...
        address account = ecrecover(calculatedHashToSign, v, r, s);
        if (account == maker) {
            return true;
        }
        return false;
    ...
    function executeStaticCall(
        ...
        (uint256 rs, bool ret) = staticCallUint(
            order.staticTarget,
            encodeStaticCall(
                order,
                call,
    ...
    function executeCall(
        ...
        address maker,
        Call memory callStruct
    ) internal {
        /* Assert target exists. */
        require(exists(callStruct.target), "Call target does not exist");
        /* Typecast. */
        OwnableDelegateProxy delegateProxy = getProxy(registry, maker);
        /* Typecast. */
        AuthenticatedProxy proxy = AuthenticatedProxy(address(delegateProxy));
        /* Execute order. */
        (bool result, bytes memory message) = proxy.proxy(
            callStruct.target,
            callStruct.howToCall,
            callStruct.data
        );
        ...
        require(result, string(message));
    }

    function getProxy(ProxyRegistryInterface registry, address maker)
        internal

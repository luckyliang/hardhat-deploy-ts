...
/**
 * @title AuthenticatedProxy Ownable代理实现合约
...
contract AuthenticatedProxy is TokenRecipient, OwnedUpgradeabilityStorage {
    ...
    function proxy(
        ...
        HowToCall howToCall,
        bytes memory data
    ) public returns (bool result, bytes memory message) {
        require(
        ...
        if (howToCall == HowToCall.Call) {
            (result, message) = dest.call(data);
        } else if (howToCall == HowToCall.DelegateCall) {
            (result, message) = dest.delegatecall(data);
        }
    ...
    function proxyAssert(
        ...
        (bool res, bytes memory message) = proxy(dest, howToCall, data);
        if (!res) {
            assembly {
                revert(add(message, 32), mload(message))
            }
        }
        require(res, string(message));
    }
}

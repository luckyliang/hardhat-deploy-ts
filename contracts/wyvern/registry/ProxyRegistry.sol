...
/**
 * @title ProxyRegistry 注册代理合约
 * @author 7seas Developers
 */
contract ProxyRegistry is Ownable, ProxyRegistryInterface {
    /* Transfer access */
    event ProxyChanged(address from, address old, address newProxy);
    ...
    /* Delay period for adding an authenticated contract.
       This mitigates a particular class of potential attack on the 7seas DAO (which owns this registry) - if at any point the value of assets held by proxy contracts exceeded the value of half the WYV supply (votes in the DAO),
       a malicious but rational attacker could buy half the 7seas and grant themselves access to all the proxy contracts. A delay period renders this attack nonthreatening - given two weeks, if that happened, users would have
       plenty of time to notice and transfer their assets.
    */
    uint public DELAY_PERIOD = 1 days;
        ...
        returns (OwnableDelegateProxy proxy)
    {
        ...
        emit ProxyChanged(msg.sender, address(0), address(proxy));
        return proxy;
    }
        ...
        returns (OwnableDelegateProxy proxy)
    {
        ...
        proxies[user] = proxy;
        emit ProxyChanged(user, address(0), address(proxy));
        return proxy;
    }
    ...
    function transferAccessTo(address from, address to) public {
        ...
        emit ProxyChanged(from, address(proxy), address(0));
        emit ProxyChanged(to, address(0), address(proxy));
    }
}

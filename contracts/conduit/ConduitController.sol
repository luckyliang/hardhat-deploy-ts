...
contract ConduitController is ConduitControllerInterface {
     ...
     *         相同的conduitKey 不能重复创建已经存在的conduit
     * @param conduitKey   The conduit key used to deploy the conduit. Note that
     *                     the first twenty bytes of the conduit key must match
     *                     the caller of this contract.
     *                     用于部署conduit(管道)的key，前20个字节必须是此合约调用者的地址
     * @param initialOwner The initial owner to set for the new conduit. 管道拥有者
     *
    ...
    function updateChannel(
        address conduit,     
        address channel, //该cahnnel（市场合约地址Seport.address） 可以打开或关闭conduit
        bool isOpen      //是否打开
    ) external override {

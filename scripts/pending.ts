...
const addresses = {
    ...
    me: "0xDE7C2b83352c64353AF810910685636Eb3b36FAc"
}

const mnemonic = fs.readFileSync("./mnemonic.txt").toString().trim();

const provider = new ethers.providers.WebSocketProvider("wss://thrilling-fabled-brook.bsc.discover.quiknode.pro/e5c165f356d50e17a28627352acd2b79e9022c36/")
...
factory.on("PairCreated", async (token0: any, token1: any, addressPair: any) => {

    const pancakeSwapLink = `https://pancakeswap.finance/swap?inputCurrency=${token0}&outputCurrency=${token1}#/swap`
    console.log(`
        ~~~~~~~~~~~~~~~~~~
        New pair detected
        ~~~~~~~~~~~~~~~~~~
        ...
        pancakeSwapLink = ${pancakeSwapLink}
    `);                                

    // This block ensures we pay with WBNB
    let buyToken, sellToken;
    if(token0 === addresses.USDT) {
        buyToken = token0;
        sellToken = token1;
    }
    if (token1 === addresses.USDT) {
        buyToken = token1;
        sellToken = token0;
    }
